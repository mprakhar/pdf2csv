...
def getEnergySimple(dict_PP, PPName):
    ...
    fileName = os.path.join( "Data", "Input", "DailyPower", "daily010119.pdf")
...
def getEnergy(fileName, dict_PP, PPName):
    ...
    # theremay be error because page ordering is not always consistent. experiment with the next page
        ...
        print("error due to inconsistent format, trying the next page")
...
def saveEnergycsv(powerConsumptionPath, saveCSVPath, dict_PP, PPName):
    ls = []
    for fileName in glob(powerConsumptionPath):# os.path.join( "Data", "Emission", "DailyPower", "daily*.pdf")):
        date = "20" + fileName[-6:-4]+ "-" + fileName[-8:-6]+ "-" + fileName[-10:-8]
        #print("fileName", fileName, date)
        
		# 2019 onwards data ned a different treatment
    ...
    df_PP.to_csv(os.path.join(saveCSVPath,  "DayEnergy_"+PPName + ".csv"))
